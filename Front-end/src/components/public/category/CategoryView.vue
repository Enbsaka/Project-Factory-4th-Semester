<template>
  <section class="py-10 px-6 md:px-20">
    ...
    <div v-if="subcategorias.length" class="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
      <div v-for="sub in subcategorias" :key="sub.id" class="cursor-pointer border rounded-xl hover:shadow-md transition p-3" @click="selecionarSubcategoria(sub)">
    ...
    <div v-if="produtos.length" class="mt-10">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
        <div v-for="prod in produtos" :key="prod.id" class="border rounded-xl hover:shadow-lg transition p-3 flex flex-col items-center">
          <img :src="getImagemProduto(prod)" alt="Produto" class="w-[180px] h-[180px] object-cover rounded-xl mb-3" />
